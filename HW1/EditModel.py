...
class EditModel(object):
  ...
  def insertEdits(self, word):
    ...
    # This function is essentially the deleteEdits function in reverse as the professor showed above.
    # Most of the code structure will resembel that as above. 
    # TODO: write this
    ...
    ret = []
    #We will use a for loop and xrange to create values as we require them
    wordlen = len(word)
    alphalen = len(EditModel.ALPHABET) 
    if wordlen <= 0:
      return 0
    for i  in xrange(0,wordlen):
      #corrupted signal will be the letter in current index position. 
      #starting at 0 is better than starting at 1. so no need to i-1
      corruptLetters = word[i]
      for j in xrange(0, alphalen):
          correctLetters = corruptLetters+EditModel.ALPHABET[j]
          # obtaining next character using the second for loop
          correction= str((word[1:i+1]) + str(EditModel.ALPHABET[j]) + str(word[i+1:]))
          ret.append(Edit(correction, corruptLetters, correctLetters))
    return ret
  ...
  def transposeEdits(self, word):
    ...
    for i in xrange(0, len(word)-1):
      ...
      #The corrected word deletes character i (and lacks the start symbol)
      if i==len(word)-1:
        correction = "%s%s" % (word[0:i], correctLetters)
      else:
        correction = "%s%s%s" % (word[0:i], correctLetters, word[i+2:-1])
      ret.append(Edit(correction, corruptLetters, correctLetters))
