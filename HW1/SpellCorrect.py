...
class SpellCorrect:
  ...
  def correctSentence(self, sentence):
    ...
    for i in xrange(1, len(sentence) - 1): #ignore <s> and </s>
      ...
      editProbabilities = self.editModel.editProbabilities(original_word)
      # ^ incase sentence has no words  
      #initialize edit probabilities
      for word, new_word in editProbabilities: 
        sentence[i] = word
        #compute new score based off new word which we're considered to replace
        new_score = new_word + self.languageModel.score(sentence)
        #the following if loop will test all probabilities and go with the best one
        if(new_score > bestScore):
          bestScore = new_score
          bestSentence = sentence[:]
        sentence[i] = original_word  
    return bestSentence
